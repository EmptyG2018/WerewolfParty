...
| 狼王 | 狼人 | 临终一击 | 被狼人击杀时可开枪带走一人，被毒或被投票出局不能发动 |
| 村民 | 好人 | 无 | 普通村民 |
...
### [2026-05-29] 配置界面三阵营优化
- **类型**: 变更
- **内容**:
  1. 配置界面角色分为3类：狼人阵营、神职阵营、平民阵营
  2. 快速预设改为卡片选择，详情展示三阵营人数与角色分布
  3. 自定义模式按三阵营分组，顶部实时显示各阵营人数
- **影响范围**: CreateRoom.tsx（UI 重构）

### [2026-05-29] 神民同体与屠边规则
- **类型**: 新增
- **内容**:
  1. 新增屠边胜负判定：狼人阵营可屠神（所有神职出局）或屠民（所有平民出局）获胜
  2. 支持配置神民同体角色（猎人、守卫），死亡时同时计入神职死亡数和平民死亡数
  3. 神民同体影响屠边判定平衡，使狼人更难达成屠边
- **影响范围**: shared/index.ts（RoomConfig.hybridRoles）、GameManager.ts（checkWinner 屠边逻辑）、CreateRoom.tsx（神民同体配置 UI）

### [2026-05-29] 新增狼王角色与预设调整
- **类型**: 新增
- **内容**:
  1. 新增狼王角色：被狼人击杀时可开枪带走一人，被毒或被投票出局不能发动
  2. 9人标准局调整为：预言家、女巫、猎人、3狼人、3村民（移除守卫）
  3. 12人进阶局调整为：预言家、女巫、猎人、守卫、狼王、3普狼、4村民
  4. 自定义模式支持配置狼王角色（狼人阵营分组）
  5. 游戏逻辑支持狼王死亡触发开枪阶段（WOLF_KING_SHOOT）
- **影响范围**: shared/index.ts（Role.WOLF_KING、GamePhase.WOLF_KING_SHOOT）、GameManager.ts（狼王逻辑）、Game.tsx（狼王阶段 UI）、CreateRoom.tsx（狼王配置）、gameStore.ts（wolfKingShoot action）
